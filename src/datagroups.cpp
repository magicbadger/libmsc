...
vector<unsigned char> Datagroup::encode()
{
	...
	// user access field
	bitset<24> useraccess_field_bits(segment.getObject().getTransportId() + // transport ID (16)
									(2 << 16) + // user access length indicator (4)
									(1 << 20) + // transport ID flag (1)
									(0 < 21) + // RFA (3)
									(segment.getIndex() << 24) + // segment number (15)
									(segment.isLast() << 25)); // last segment (1)
	...
	// calculate CRC
	bytes = bytes + calculate_crc(bytes);
